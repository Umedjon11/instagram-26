...
import { axiosRequest, clearStorage, isLogined } from "@/utils/axios"
...
const SideBar = () => {
  ...
  return (
    <div className={`pr-4 transition-all duration-400 h-screen ${pathname == "/messages" || openNotifitactions || openSearch ? "w-[5.5%] pl-5" : "w-[18%] pl-6"} border max-h-screen pt-[4vh] flex flex-col gap-[1vh] fixed bg-white dark:bg-black`}>
      <div className={openSearch || openNotifitactions || pathname == "/messages" ? "hidden" : "block mb-[4vh]"}><InstagramTextLogo width="153" height="50" /></div>
      <div className={!openSearch && !openNotifitactions && pathname != "/messages" ? "hidden" : "mb-[5vh] m-[0_auto]"}><InstagramIcon size={30} /></div>
      <Link onClick={() => {
        setOpenNotifitactions(false)
        setOpenSearch(false)}} className={`flex ${pathname == "/" ? "font-bold" : "font-semibold"} ${pathname == "/messages" || openNotifitactions || openSearch ? "p-[1.5vh]" : "p-[1.7vh_20px]"} transition-all duration-300 rounded-md w-full hover:bg-[#F3F3F3] dark:hover:bg-[#25282C] gap-3 items-center`} href="/">
      ...
      <button onClick={() => {
        setOpenNotifitactions(false)
        setOpenSearch(true)}} className={`flex ${openSearch ? "font-bold" : "font-semibold"} ${pathname == "/messages" || openNotifitactions || openSearch ? "p-[1.5vh]" : "p-[1.7vh_20px]"} transition-all duration-300 rounded-md w-full cursor-pointer hover:bg-[#F3F3F3] dark:hover:bg-[#25282C] gap-3 items-center`}>
        {!openSearch ?
      ...
      <Link onClick={() => {
        setOpenNotifitactions(false)
        setOpenSearch(false)}} className={`flex ${pathname == "/interested" ? "font-bold" : "font-semibold"} ${pathname == "/messages" || openNotifitactions || openSearch ? "p-[1.5vh]" : "p-[1.7vh_20px]"} transition-all duration-300 rounded-md w-full hover:bg-[#F3F3F3] dark:hover:bg-[#25282C] gap-3 items-center`} href="/interested">
      ...
      <Link onClick={() => {
        setOpenNotifitactions(false)
        setOpenSearch(false)}} className={`flex ${pathname == "/reels" ? "font-bold" : "font-semibold"} ${pathname == "/messages" || openNotifitactions || openSearch ? "p-[1.5vh]" : "p-[1.7vh_20px]"} transition-all duration-300 rounded-md w-full hover:bg-[#F3F3F3] dark:hover:bg-[#25282C] gap-3 items-center`} href="/reels">
      ...
      <Link onClick={() => {
        setOpenNotifitactions(false)
        setOpenSearch(false)}} className={`flex ${pathname == "/messages" ? "font-bold" : "font-semibold"} ${pathname == "/messages" || openNotifitactions || openSearch ? "p-[1.5vh]" : "p-[1.7vh_20px]"} transition-all duration-300 rounded-md w-full hover:bg-[#F3F3F3] dark:hover:bg-[#25282C] gap-3 items-center`} href="/messages">
      ...
      <button onClick={() => {
        setOpenSearch(false)
        setOpenNotifitactions(true)}} className={`flex ${openSearch ? "font-bold" : "font-semibold"} ${pathname == "/messages" || openNotifitactions || openSearch ? "p-[1.5vh]" : "p-[1.7vh_20px]"} transition-all duration-300 rounded-md w-full cursor-pointer hover:bg-[#F3F3F3] dark:hover:bg-[#25282C] gap-3 items-center`}>
        {!openNotifitactions ?
      ...
      <button onClick={() => setOpenAdd(true)} className={`flex font-semibold ${pathname == "/messages" || openNotifitactions || openSearch ? "p-[1.5vh]" : "p-[1.7vh_20px]"} transition-all duration-300 rounded-md w-full hover:bg-[#F3F3F3] dark:hover:bg-[#25282C] gap-3 items-center`}>
      ...
      <Link onClick={() => {
        setOpenNotifitactions(false)
        setOpenSearch(false)}} className={`flex ${pathname == "/profile" ? "font-bold" : "font-semibold"} ${pathname == "/messages" || openNotifitactions || openSearch ? "p-[1.5vh]" : "p-[1.7vh_20px]"} transition-all duration-300 rounded-md w-full hover:bg-[#F3F3F3] dark:hover:bg-[#25282C] gap-3 items-center`} href="/profile">
        {userProfile?.image ? (<img alt="profile photo" width={30} className={`${openSearch || openNotifitactions || pathname == "/messages" ? "h-6 w-7" : "h-8 w-8"} rounded-full ${pathname == "/profile" ? "border-2 border-black dark:border-white p-px" : ""}`} height={30} src={ "https://instagram-api.softclub.tj/images/"+userProfile?.image } />) : (<p></p>)}
        <span className={openSearch || openNotifitactions || pathname == "/messages" ? "hidden" : ""}>Profile</span>
      </Link>
      <a className={`flex font-semibold ${pathname == "/messages" || openNotifitactions || openSearch ? "p-[1.5vh]" : "p-[1.7vh_20px]"} mt-auto transition-all duration-300 rounded-md w-full hover:bg-[#F3F3F3] dark:hover:bg-[#25282C] gap-3 items-center`} target="_blank" href="https://www.threads.com/?xmt=AQF0yHK3oizGZF3BDTLYQMCFIqMZtcsRVyIU3-fX7hwjDLs&next=%2F">
    ...
    <DropdownMenu>
      ...
      <button className={`flex font-semibold ${pathname == "/messages" || openNotifitactions || openSearch ? "p-[1.5vh]" : "p-[1.7vh_20px]"} transition-all duration-300 rounded-md w-full hover:bg-[#F3F3F3] dark:hover:bg-[#25282C] gap-3 items-center`}>
