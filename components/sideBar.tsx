...
const SideBar = () => {
  ...
  return (
    <div className={`pr-4 transition-all duration-400 h-screen ${pathname == "/messages" || openNotifitactions || openSearch ? "w-[5.5%] pl-5" : "w-[18%] pl-6"} border h-screen max-h-screen flex flex-col gap-[1vh] fixed bg-white dark:bg-black`}>
      ...
      <button onClick={() => {
        setOpenNotifitactions(false)
        setOpenSearch(!openSearch)}} className={`flex ${openSearch ? "font-bold" : "font-semibold"} ${pathname == "/messages" || openNotifitactions || openSearch ? "p-[1.4vh_1.7vh]" : "p-[1.4vh_20px]"} transition-all duration-300 rounded-md w-full cursor-pointer hover:bg-[#F3F3F3] dark:hover:bg-[#25282C] gap-3 items-center`}>
        {!openSearch ?
